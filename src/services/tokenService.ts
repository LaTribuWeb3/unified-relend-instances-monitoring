...
async function retryWithBackoff<T>(fn: () => Promise<T>, label: string, maxRetries = 3, delayMs = 1500): Promise<T> {
  let attempt = 0;
  while (attempt < maxRetries) {
    try {
      ...
      attempt++;
      const is429 = error?.message?.includes('429') || error?.details?.includes('Your app has exceeded its compute unit per seconds capacity');
      if (attempt < maxRetries && is429) {
        console.warn(`[Retry] ${label} attempt ${attempt} failed due to rate limit. Retrying in ${delayMs / 1000}s...`, error);
        await new Promise(res => setTimeout(res, delayMs));
      } else {
        if (attempt >= maxRetries) {
          console.error(`[Retry] ${label} failed after ${maxRetries} attempts.`, error);
        }
        throw error;
      }
    }
...
export const tokenService = {
  async fetchTokenData(): Promise<TokenData[]> {
    ...
    const tokenDataPromises = tokenDefinitions.map(async (tokenDefinition) => {
      ...
      try {
        name = await retryWithBackoff(() => tokenDataComputer.name(), `name(${tokenDefinition.L1WrappedTokenAddress})`);
        symbol = await retryWithBackoff(() => tokenDataComputer.symbol(), `symbol(${tokenDefinition.L1WrappedTokenAddress})`);
        totalSupply = await retryWithBackoff(() => tokenDataComputer.totalSupply(), `totalSupply(${tokenDefinition.L1WrappedTokenAddress})`);
        totalSupplyUSDC = await retryWithBackoff(() => tokenDataComputer.totalSupplyUSDC(), `totalSupplyUSDC(${tokenDefinition.L1WrappedTokenAddress})`);
      } catch (error) {
        console.error(
